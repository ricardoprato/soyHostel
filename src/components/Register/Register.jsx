...
import FuncionAux from '../FuncionAux/FuncionAux';
...
const Register = () => {
  ...
  useEffect(() => {
    if (dataProfile) {
      setInitial((prev) => ({
        ...prev,
        nombre: dataProfile.givenName,
        lastname: dataProfile.familyName,
        mail: dataProfile.email,
      }));
    }
  }, [dataProfile]);
  console.log(initial);
      ...
      {modal ? (
        <Formik
          ...
          validate={(valores) => {
            ...
            // Validacion mail
            if (!valores.mail) {
              errores.mail = 'Please enter a email';
            } else if (
        ...
        <Popup setModal={setModal} setDataProfile={setDataProfile} />
      )}
    </div>
  );
};
